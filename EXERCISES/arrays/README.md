...
**1 -> [CopyArray.java]** Write a method accepting a double[] and returning a copy of the initial double[] (see
System.arraycopy()).
...
**2 -> [FillArray.java]** Write a method returning a double[] already filled with values (see java.util.Arrays,
...
**3 -> [DivideArray.java]** Write a method accepting a double[] and a double value returning a different double[]
...
**4 -> [DivideArrays.java]** Write a method accepting two double[] returning a different double[] representing the first
...
**5 -> [Fibonacci.java]** Write a method accepting an int and returning a long[] composed of the first n numbers of the
Fibonacci series.
...
**6 -> [BubbleSort.java]** Write two methods for ordering an int[]. The only difference between the two methods is that the
...
**7 -> [MoveZeroEnd.java]** Write a method accepting an int[] eventually containing zeros and returning an int[] in which all the zeros are at the end of the array. Maintain the relative order of the other (non-zero) elements.
...
**8 -> [Splitter.java]** Write a method accepting an int and returning the sequence of the digits as an int[].
...
**9 -> [IsEverywhere.java]** We'll say that a value is "everywhere" in an array if for every pair of adjacent elements in the
...
**10 -> [TenRun.java]** For each multiple of 10 in the given array, change all the values following it to be that multiple of
...
**11 -> [FizzBuzz.java]** Consider the series of numbers beginning at **start** and running up to but not including **end**.
...
**12 -> [MatchUp.java]** Given two int[] v1 and v2 of the same length, for every element in v1, consider the corresponding
...
**13 -> [CanBalance.java]** Given a non-empty array, return true if there is a place to split the array so that the sum of the
...
**14 -> [SearchArray.java]** Given a String[] and a String return true if the String is contained within the String[].
...
**15 -> [DeepCopy]** Write a method accepting an int[][] and returning a deep copy of the original.
